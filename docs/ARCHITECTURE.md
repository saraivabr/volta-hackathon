...
flowchart LR
  ...
  API --> TX[Telnyx TeXML]
  TX --> PSTN[Public phone network]
  PSTN --> SIP[OpenAI Realtime SIP]
...
- Telnyx owns the public-network leg and hands media to the OpenAI Realtime SIP endpoint; no bridge of ours sits in that path.
