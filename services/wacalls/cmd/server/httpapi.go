...
func (s *server) routes() http.Handler {
	...
	mux.HandleFunc("POST /api/sessions/{sid}/agent-calls", s.handleStartAgentCall)
...
func (s *server) withSecurity(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/health" && !s.config.authorize(r.Header.Get("Authorization")) {
...
func (s *server) handleHealth(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, map[string]any{"ok": true, "service": "volta-wacalls", "model": s.config.model})
}
...
func (s *server) handleSessionGet(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, sess.detail())
}
...
func (s *server) handleStartAgentCall(w http.ResponseWriter, r *http.Request) {
	if sess := s.sessionByID(w, r.PathValue("sid")); sess != nil {
		s.doStartAgentCall(sess, w, r)
	}
}

func (s *server) handleSendMessage(w http.ResponseWriter, r *http.Request) {
	if sess := s.sessionByID(w, r.PathValue("sid")); sess != nil {
		s.doSendMessage(sess, w, r)
	}
}
...
func (s *server) doWebRTC(sess *Session, w http.ResponseWriter, r *http.Request) {
	...
	bridge, answer, err := newBridge(body.SDPOffer, s.log, s.config.webRTCPublicIP)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
		return
	}
